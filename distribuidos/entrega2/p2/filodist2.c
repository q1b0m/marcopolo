...
void * comunicaciones(void)
{
  	...
  	if (setsockopt(sockserver, SOL_SOCKET, SO_REUSEADDR,
  		...
  		perror("setcokopt(SO_REUASEADDR) failed");
	

	if (setsockopt(sockserver, SOL_SOCKET, SO_REUSEPORT,
			(char*)&reuse, sizeof(reuse)) < 0)

  		perror("setcokopt(SO_REUASEPORT) failed");
  	...
  	while (1)
  	{
  		...
  		//APARTADO2
  		// 6 - esperar token
  		ret = read(sockant, &tokenBAK, sizeof(unsigned char));
  		...
  		if (ret==1) // si se leyó bien
  		{
  			...
  			//APARTADO 2
  			if (tokenBAK != token) {
  				fprintf(stderr, "Filosofo %d: nuevo token ", idfilo);
  				BYTE_TO_BIN(token)
  			}
